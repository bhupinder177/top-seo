...
class Common extends CI_Model {
 ...
 public function getfreelancer($whr)
 {
  ...
  public function getallteam($whr,$employeename,$skill,$start,$perPage)
  {
    $this->db->select('us.id as u_id,us.email,us.deleted,us.is_active,us.access,um.name,um.desig,um.address1,um.rep_ph_num as phone,um.maxPrice,um.logo,c.code,c.symbol,um.salary,um.perday,um.perHour,um.employeeCode');
    $this->db->from('user as us');
    $this->db->join('user_meta as um','um.u_id = us.id');
    $this->db->join('currency as c','c.id = um.currencyId','left');
    $this->db->join('user_services as s','s.u_id = us.id','left');
    $this->db->join('services as se','se.id = s.servicesId','left');
    $this->db->where($whr);
    if($employeename != '')
    {
      $this->db->like('um.name',$employeename,'both');
    }
    if($skill != '')
    {
      $this->db->like('se.name',$skill,'both');
    }
  ...
  public function count_all_team($tbl,$whr,$employeename,$skill)
  {
    $this->db->from('user as u');
    $this->db->join('user_meta as um','um.u_id = u.id');
    $this->db->join('user_services as s','s.u_id = u.id','left');
    $this->db->join('services as se','se.id = s.servicesId','left');
    $this->db->where($whr);
    if($employeename != '')
    {
...
{
  $this->db->select('us.id as u_id,us.email,us.is_active,us.access,um.name,um.desig,um.address1,um.rep_ph_num as phone,um.maxPrice,um.logo,um.about,um.currencyId,um.perHour,um.department');
  $this->db->from('user as us');
  $this->db->join('user_meta as um','um.u_id = us.id');
  $this->db->where($whr);
  $this->db->order_by('um.department','Desc');
  $result = $this->db->get();
  $result = $result->result();
  return $result;
}
...
// FROM user_service_ranking where userId = 122 and servicesId = 54
 ...
 public function getallEmployeeLeave($tbl,$whr,$date,$start,$perPage)
 {
   $this->db->select('*');
   $this->db->from($tbl);
   // $this->db->join('user_meta as m','m.u_id = l.employeeId');
   // $this->db->join('leavetype as t','t.type = t.type','left');
   $this->db->where($whr);
   if($date != '')
   {
   $this->db->where('date',$date);
 ...
 public function getmanagerdsr($whr,$startdate,$enddate,$id,$employeeId,$status,$start,$perPage)
 {
 ...
 public function count_manager_dsr($whr,$startdate,$enddate,$id,$employeeId,$status)
 {
   $this->db->from('dsr as d');
   $this->db->where($whr);
   $this->db->where_not_in('employeeId',$id);
 ...
 public function getMydsr($whr,$startdate,$enddate,$status,$start,$perPage)
 {
...
public function allincome($whr,$date,$end,$client,$project,$start,$perPage)
 ...
 public function countemployeeDsr($whr,$startdate,$enddate,$status)
 {
   $this->db->from('dsr as d');
   $this->db->where($whr);
   $this->db->where("date >=",$startdate);
...
public function getGigClient($whr)
{
 ...
 public function getallsalaryteam($whr,$employeename,$date)
 {
   $this->db->select('us.id as u_id,us.email,us.deleted,us.is_active,us.access,um.name,um.desig,um.address1,um.rep_ph_num as phone,um.maxPrice,um.logo,c.code,c.symbol,um.salary,um.perday,um.perHour,um.date_created');
   $this->db->from('user as us');
   $this->db->join('user_meta as um','um.u_id = us.id');
   $this->db->join('currency as c','c.id = um.currencyId','left');
   $this->db->join('user_services as s','s.u_id = us.id','left');
   $this->db->join('services as se','se.id = s.servicesId','left');
   $this->db->where($whr);
   $this->db->where('um.date_created <=',$date);
   if($employeename != '')
   {
     $this->db->like('um.name',$employeename,'both');
   }
 ...
 public function count_all_salary_team($tbl,$whr,$employeename,$date)
 {
   $this->db->from('user as u');
   $this->db->join('user_meta as um','um.u_id = u.id');
   $this->db->join('user_services as s','s.u_id = u.id','left');
 ...
 public function salesMajorClient($whr)
 {
  $this->db->select('*');
  $this->db->from('project');
  $this->db->where($whr);
  $this->db->where("budget >=",5000);
  $this->db->limit('7');
  $result = $this->db->get();
  $result = $result->result();
  return $result;
...
public function salesActiveClient($whr)
{
 $this->db->select('*');
 $this->db->from('project');
 $this->db->where($whr);
...
public function getallEmployee($whr,$id)
{
  $this->db->select('us.id as u_id,us.email,us.is_active,um.name');
  $this->db->from('user as us');
  $this->db->join('user_meta as um','um.u_id = us.id');
  $this->db->where($whr);
  $this->db->where_not_in('us.id',$id);
...
public function countTeamEmployeeCode($whr)
{
  $this->db->from('user as us');
  $this->db->join('user_meta as um','um.u_id = us.id');
  $this->db->where($whr);
  $this->db->where('um.employeeCode !=','');
  $count = $this->db->count_all_results();
  return  $count;
}
