...
$http({
 ...
 $scope.submitcurrency = function()
 {
	...
	if($scope.currency == '' || $scope.symbol == '' || $scope.code == '')
	{
		error = true;
	}
	...
	var obj = { name:$scope.currency,code:$scope.code,symbol:$scope.symbol  }
	...
	// membership
	  ...
	  // support chat
        ...
        return function (item) {
         ...
         {
         socket.emit('join-chat', { senderId: $scope.userId,receiverId : id });
         }
         ...
         $scope.groupkey = 0;
         $scope.selectedgroupName = $scope.group[0].UserName;
         ...
         socket.emit('creategroup', { Name:$scope.groupname,UserId:$scope.userId,Type:0});
         ...
         $scope.roomId = id;
         $scope.groupkey = key;
         $scope.selectedgroupName = $scope.group[key].UserName;
         $scope.socketjoingroup($scope.roomId);
         $scope.getchat($scope.roomId);
         }
         ...
         socket.emit('message', { MSG_SENTBYNAME:$scope.username,MSG_SENTBYIMAGE:$scope.userimage,MSG_SENTBY: $scope.userId,MSG_SENTTONAME:name,  MSG_SENTTO : uid,MSG_SENTTOIMAGE:$scope.image,MSG_TEXT : msgtext,MSG_ATTACHMENT:$scope.attachment,MSG_ROOMID :$scope.roomId,MSG_SENTTOIMAGE:$scope.selectedContactImage ,MSG_DELETE : 0 ,MSG_EDIT: 0,MSG_STATUS:0,MSG_TYPE:1});
         ...
         socket.emit('groupmessage', { MSG_SENTBYNAME:$scope.username,MSG_SENTBYIMAGE:$scope.userimage, MSG_SENTBY: $scope.userId,MSG_SENTTONAME:'',  MSG_SENTTO : '',MSG_SENTTOIMAGE:'',MSG_TEXT : msgtext,MSG_ATTACHMENT:$scope.attachment,MSG_ROOMID :id,MSG_SENTTOIMAGE:'' ,MSG_DELETE : 0 ,MSG_EDIT: 0});
         angular.element('.msgtext').val('');
         $scope.attachment ='';
         $scope.getallLatestgroup();
         ...
         $scope.group = [];
         // var m = JSON.stringify({ roomId :roomId});
         // https://top-seo-sockets.herokuapp.com
         $http({
         ...
         $scope.getallLatestgroup = function ()
         {

           var dd = $scope.group[$scope.groupkey];
           $scope.group.splice($scope.groupkey,'1');
...
$scope.cexpenseerror= false;
$scope.cinvoiceerror= false;
$scope.cjobposterror= false;


$scope.get = function ()
{
$http({
url: '<?php echo base_url(); ?>admin/getcompanycustom-package',
method: "POST",
headers: {
  'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
}
...
$scope.getclientpackage = function ()
{
$http({
...
headers: {
 ...
 if(response.data.success == 'true')
 {
  $scope.cjobpostmin =response.data.result.jobpostMin;
  $scope.cjobpostmax =response.data.result.jobpostMax;
  $scope.cjobpostprice =response.data.result.jobpostPrice;
  $scope.cexpensemin =response.data.result.expenseMin;
  $scope.cexpensemax =response.data.result.expenseMax;
  $scope.cexpenseprice =response.data.result.expensePrice;
  $scope.cinvoicemin =response.data.result.invoiceMin;
  $scope.cinvoicemax =response.data.result.invoiceMax;
  $scope.cinvoiceprice =response.data.result.invoicePrice;
 }
   });
}
...
$scope.update = function()
{
 ...
 if($scope.gigmin == '' || $scope.gigmax == '' || $scope.gigprice == '' )
 {
   error = true;
 }
 ...
 if(parseInt($scope.gigmin) > parseInt($scope.gigmax))
 {
   error = true;
   $scope.gigerror = true;
 }
...
var obj = { type:1,employeeMin :$scope.employeemin,employeeMax:$scope.employeemax,employeePrice:$scope.employeeprice,projectMin:$scope.projectmin,projectMax:$scope.projectmax,projectPrice:$scope.projectprice,expenseMin:$scope.expensemin,expenseMax:$scope.expensemax,expensePrice:$scope.expenseprice,invoiceMin:$scope.invoicemin,invoiceMax:$scope.invoicemax,invoicePrice:$scope.invoiceprice,packageMin :$scope.packagemin,packageMax:$scope.packagemax,packagePrice:$scope.packageprice,testimonialMin :$scope.testimonialmin,testimonialMax:$scope.testimonialmax,testimonialPrice:$scope.testimonialprice,portfolioMin :$scope.portfoliomin,portfolioMax:$scope.portfoliomax,portfolioPrice:$scope.portfolioprice,casestudiesMin :$scope.casestudiesmin,casestudiesMax:$scope.casestudiesmax,casestudiesPrice:$scope.casestudiesprice,industryMin :$scope.industrymin,industryMax:$scope.industrymax,industryPrice:$scope.industryprice,servicesMin :$scope.servicesmin,servicesMax:$scope.servicesmax,servicesPrice:$scope.servicesprice,gigMin:$scope.gigmin,gigMax:$scope.gigmax,gigPrice:$scope.gigprice};
$http({
  ...
  if(response.data.success == "true")
  {
    $.notify("Company Package Updated Successfully ", "success");
  }
});
}
...
$scope.cupdate = function()
{
 ...
 if($scope.cjobpostmin == '' || $scope.cjobpostmax == '' || $scope.cjobpostprice == '' )
 {
   error = true;
 }

 if(parseInt($scope.cjobpostmin) > parseInt($scope.cjobpostmax))
 {
   error = true;
   $scope.cjobposterror= true;
 }
 else
 {
   $scope.cjobposterror= false;
 ...
 if($scope.cinvoicemin == '' || $scope.cinvoicemax == '' || $scope.cinvoiceprice == '' )
 {
   error = true;
 }

 if(parseInt($scope.cinvoicemin) > parseInt($scope.cinvoicemax))
 {
   error = true;
   $scope.cinvoiceerror = true;
 }
 else
 {
   $scope.cinvoiceerror = false;
 ...
 if($scope.cexpensemin == '' || $scope.cexpensemax == '' || $scope.cexpenseprice == '' )
 {
   error = true;
 }

 if(parseInt($scope.cexpensemin) > parseInt($scope.cexpensemax))
 {
   error = true;
   $scope.cexpenseerror = true;
 }
 else
 {
   $scope.cexpenseerror = false;
...
var obj = { type:2,jobpostMin :$scope.cjobpostmin,jobpostMax:$scope.cjobpostmax,jobpostPrice:$scope.cjobpostprice,expenseMin:$scope.cexpensemin,expenseMax:$scope.cexpensemax,expensePrice:$scope.cexpenseprice,invoiceMin:$scope.cinvoicemin,invoiceMax:$scope.cinvoicemax,invoicePrice:$scope.cinvoiceprice};
$http({
  url: '<?php echo base_url(); ?>admin/customPackageUpdate',
  method: "POST",
  data :obj,
  ...
  if(response.data.success == "true")
  {
    $.notify("Client Package Updated Successfully ", "success");
  }
});
}
