...
///////jobs
   ...
   var name = angular.element("#industrys").val();
	...
	$scope.totalbudget = function($event)
	{
	  $scope.budget = $scope.estimationHours * $scope.estimationHourlyPrice;
	}
	...
	$scope.submitjob = function ()
  {
   ...
   //console.log($scope.description);
    if($scope.jobtitle == '' || $scope.description == '' || $scope.jobservices == '' || $scope.jobskill.length == 0 || $scope.jobrequiredf =='' || $scope.jobindustries.length == 0)
    {
      error = true;
    }

		if($scope.estimationHours =='' || $scope.estimationHourlyPrice =='' || $scope.budget =='' || $scope.currency == '')
		{
			error = true;
		}
...
// edit job end
		$scope.UpdateJob = function()
		{
		 ...
		 $scope.description = CKEDITOR.instances['description1'].getData();
		 //console.log($scope.description);
			if($scope.jobId == '' || $scope.jobtitle == '' || $scope.description == '' || $scope.jobservices == '' || $scope.jobskill.length == 0 || $scope.jobrequiredf =='' || $scope.jobindustries.length == 0)
			{
				error = true;
			}
			...
			var m = JSON.stringify({ jobId:$scope.jobId, jobtitle : $scope.jobtitle ,required:$scope.jobrequiredf,description : $scope.description,industry:$scope.jobindustries,service:$scope.jobservices,skill: $scope.jobskill,image:$scope.image ,totalAmount : $scope.total ,milestone : $scope.milestones,estimatehours:$scope.estimationHours,hourlyPrice: $scope.estimationHourlyPrice ,budget:$scope.budget });
			angular.element(".preloader").show();
			 $http({
				 ...
				 if(response.data.message == "true")
					{
						...
						$scope.description ='';
					 $scope.jobSubmit = false;
					 $.notify("Job Updated Successfuly", "success");
		       ...
		       if(response.data.message == "true")
		        {
		          ...
		          $.notify("Milestone Successfully Created", "success");
			 ...
			 $scope.requestsend = function()
			 {
						...
						$http({
							...
							if(response.data.message == "true")
							{
								if($scope.status == 3)
								{
									$.notify("Request Accept Successfully", "success");
	 ...
	 $scope.contractstart = function()
	 {
			 ...
			 $http({
				...
				angular.element(".preloader").hide();
				 ...
				 if(response.data.message == "true")
				 {
					$.notify("Milestone Start Successfully", "success");
				 }
	         ...
	         if(response.data.message == "true")
	          {
	            angular.element("#hire").modal('hide');
	             $scope.getproposal();
	            $.notify("Offer Send Successfully", "success");
        ...
        if(pclass == "form-control phonenumber ng-pristine ng-valid ng-not-empty error11 ng-touched")
           {
             return false;
       		...
       		var obj = {  name : $scope.profile.name , zip : $scope.profile.zip ,logo : $scope.profile.logo , rep_ph_num : $scope.profile.phone ,country : $scope.profile.country,state : $scope.profile.state ,city : $scope.profile.city,address1 : $scope.profile.address1,address2 : $scope.profile.address2,about : $scope.profile.about,website : $scope.profile.website,skype : $scope.profile.skype,facebookLink : $scope.profile.facebookLink , linkedlnLink : $scope.profile.linkedlnLink ,desig: $scope.profile.desig ,c_name: $scope.profile.c_name ,seoTitle:$scope.profile.seoTitle,seoDescription:$scope.profile.seoDescription,countryCode:$scope.countrycode,countryClass:cl,company_logo:$scope.profile.company_logo };
