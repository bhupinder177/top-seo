  ...
  $('body').on('click', '.buy_now', function(e){
    ...
    "handler": function (response){
          $.ajax({
            ...
            dataType: 'json',
            data: {
                razorpay_payment_id: response.razorpay_payment_id,totalAmount:totalAmount,expense:expense,invoice:invoice,jobpost:jobpost,totalamount:totalAmount
            },
            dataType    : "json",
          beforeSend  : function ()
          {
...
///////jobs
   ...
   app3.controller('myCtrt3', function($scope,$window,$http,$interval) {
      ...
      $scope.jobskill1 =[];
			...
			$scope.refindustry1 =[];
			$scope.jobservices1 ='';
			$scope.jobindustries1 = [];
			$scope.jobrequiredf1='';
			$scope.industry1 = [];
			$scope.services1 = [];
			$scope.EditjobData=[];
      $scope.jobId ='';
			$scope.estimationHours1 ='';
			$scope.estimationHourlyPrice1='';
			$scope.budget1='';
			$scope.currency1 ='';
			$scope.allcurrency = [];
			$scope.typingLENGTH =800;
      ...
      $scope.repost = function(id)
      {
        ...
        $http({
        url: '<?php echo base_url(); ?>client-repostJob',
        method: "POST",
        data: m,
        headers: {
         'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
          ...
          if(response.data.message == "true")
           {
             ...
             CKEDITOR.instances['description'].setData($scope.EditjobData.jobDescription);
      ...
      $scope.jobcloseconfirm = function (id)
      {
        $scope.jobId = id;
        $('#closejobconfirm').modal('show');
      }

      $scope.jobclose = function()
      {
        var obj = { jobId :$scope.jobId };
        $http({
          url: '<?php echo base_url(); ?>client-jobClose',
          method: "POST",
          data :obj,
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
          ...
          if(response.data.message == "true")
          {
            angular.element('#closejobconfirm').modal('hide');
            $.notify("Job Post Closed Successfully", "success");
            $scope.job();
          }
          else if(response.data.error == "true")
          {
            $.notify("Someone already Accepted proposal", "error");
            angular.element('#closejobconfirm').modal('hide');
          }
        });
      }
      ...
      $scope.showcreate = function()
      {
        $http({
          ...
          if($scope.plan.jobpost != $scope.total1 && $scope.plan.jobpost >= $scope.total1)
          {
       ...
       $scope.clickUpgrade = function()
       {
         ...
         window.open('<?php echo base_url(); ?>client-membership/', '_blank');
      ...
      $http({
        ...
        {
          $.notify("Someone already submitted proposal", "error");
          angular.element('#confirm').modal('hide');
        }
      });
      }
					 ...
					 if(response.data.message == "true")
						{
							...
							$scope.EditjobData = response.data.result;
							$scope.jobId = $scope.EditjobData.jobId;
							$scope.jobtitle1 = $scope.EditjobData.jobTitle;
							CKEDITOR.instances['description1'].setData($scope.EditjobData.jobDescription);
	            $scope.total1 = $scope.EditjobData.jobAmount;
							$scope.jobrequiredf1 = $scope.EditjobData.jobRequiredFreelancer;
							$scope.jobservices1 = $scope.EditjobData.servicesId;
							$scope.image1 = $scope.EditjobData.jobAttchment;
							$scope.estimationHours1 =$scope.EditjobData.jobEstimateHours;
							$scope.estimationHourlyPrice1=$scope.EditjobData.jobEstimateHourlyPrice;
							$scope.budget1= $scope.EditjobData.jobBudget;
							$scope.currency1 = $scope.EditjobData.currencyId;
              $scope.type1 = $scope.EditjobData.jobType;
...
// edit job end
		$scope.UpdateJob = function()
		{
			$scope.jobSubmit = true;
			var error = false;
		 $scope.description1 = CKEDITOR.instances['description1'].getData();
		 //console.log($scope.description);
			if($scope.jobId == '' || $scope.jobtitle1 == '' || $scope.description1 == '' || $scope.budget1 =='' || $scope.currency1 == '' || $scope.jobservices1 == '' || $scope.jobskill.length == 0 || $scope.jobrequiredf1 =='' || $scope.jobindustries.length == 0)
			{
				error = true;
			}
      ...
      if($scope.type1 == 2)
      {
        if($scope.estimationHours1 =='' || $scope.estimationHourlyPrice1 =='')
        {
          error = true;
         }
      }

      if($scope.jobrequiredf1 == 0)
      {
        error = true;
      }
      if($scope.budget1 == 0)
      {
        error = true;
			...
			var m = JSON.stringify({ jobType:$scope.type1,jobId:$scope.jobId, jobtitle : $scope.jobtitle1 ,required:$scope.jobrequiredf1,description : $scope.description1,industry:$scope.jobindustries1,service:$scope.jobservices1,skill: $scope.jobskill,image:$scope.image1 ,totalAmount : $scope.total1 ,milestone : $scope.milestones1,estimatehours:$scope.estimationHours1,hourlyPrice: $scope.estimationHourlyPrice1 ,budget:$scope.budget1,currencyId:$scope.currency1 });
			angular.element(".preloader").show();
			 $http({
	     ...
	     if(error == true)
	      ...
	      angular.element(".preloader").show();
	       ...
	       headers: {
            ...
            {
              $.notify("Offer not Sent", "error");

            }
            else if(response.data.already == "true")
            {
              $.notify("Offer Already Sent", "error");
            }
	           });
	    }
...
<?php if(isset($proposalId))
	...
	////upload attchment image
      ...
      $scope.phonetime = 1;
       ...
       if($scope.profile.company_logo)
       {
         $('#logo').addClass('removeimageValue');
          ...
          if($scope.profile.rep_ph_num.length < 10 || $scope.profile.rep_ph_num.length > 11 )
          {
            error = true;
          }
          // var ppclass = pclass.split(" ");
          // if(ppclass.includes("error11"))
          // {
          //    return false;
          //  }
     ...
     $scope.receivedkeyup = function (val,id,gigbuyId)
     {
      var typingTimer = (new Date()).getTime()
      ...
      $interval(function(){
        ...
        var timeDiff    = typingTimer - $scope.lastTypingTime;
        if (timeDiff   >= $scope.typingLENGTH && $scope.typing) {
          $scope.receivedSave(val,id,gigbuyId);
          $scope.typing = false;
        }
      },$scope.typingLENGTH);
     }

     $scope.receivedSave = function(val,id,gigbuyId)
     {
       var obj = {  receivedAmount:val,user_gig_buyId:gigbuyId,planId:id };

      $http({
        url: '<?php echo base_url(); ?>client-gigpayment',
        method: "POST",
        data :obj,
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
     ...
     if(response.data.success == "true")
     {
          ...
          // $scope.getuserreview();
           ...
           var obj = { user_gig_buyId : $scope.gig.user_gig_buyId,overall:$scope.rating,freelancerId:$scope.gig.userId,communication : $scope.communication,skill:$scope.skill,deadline:$scope.deadline,quality:$scope.quality,cooperation:$scope.cooperation,review:$scope.review,rehire:$scope.rehire,cost:$scope.cost,availability:$scope.availability,total:$scope.overall }
           angular.element(".preloader").show();
           $http({
           url: '<?php echo base_url(); ?>client-gigreview',
           method: "POST",
           data: obj,
           headers: {
             'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
          ...
          $scope.getuserreview = function()
           ...
           $scope.changeclientStatus = function($event)
           {
             ...
             var a = angular.element($event);
             var b = a[0].selectedOptions[0];
             var key = b.getAttribute('data-id');
             var key1 = b.getAttribute('data-id1');
             var todoId = $scope.gig.milestone[key].task[key1].id;
             if(status != '')
            {
              var obj = {  user_gig_buyId : $scope.gig.user_gig_buyId,clientStatus:status,todoId:todoId,gigId:$scope.gig.gigId };

              $http({
               url: '<?php echo base_url(); ?>client-gigStatusUpdate',
               method: "POST",
               data: obj,
               headers: {
                 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
           ...
           app18.directive('convertToNumber', function() {
  return {
    require: 'ngModel',
      ...
      ngModel.$formatters.push(function(val) {
        return val != null ? '' + val : null;
           ...
           app18.controller('myCtrt18', function($scope,$window,$http,$interval) {
             ...
             angular.element(".preloader").show();
             var obj = {  expense :$scope.expense,amount:$scope.amount,paidby:$scope.paidby,status:$scope.status,currencyId:$scope.currency,recurring:$scope.recurring,date:$scope.date,remarks:$scope.remarks };
             ...
             $http({
               ...
               if(response.data.message == "true")
               {
                 ...
                 $scope.remarks1 = $scope.editexpense1.remarks;
             ...
             angular.element(".preloader").show();
             var obj = {  id:$scope.editexpense1.id,expense :$scope.expense1,amount:$scope.amount1,date:$scope.date1,paidby:$scope.paidby1,status:$scope.status1,currencyId:$scope.currency1,recurring:$scope.recurring1,remarks:$scope.remarks1 };
             ...
             $scope.getclone = function (id)
             {
               ...
               $http({
                 ...
                 if(response.data.message == "true")
                 {
                   ...
                   $scope.remarks = $scope.editexpense.remarks;
        ...
        {
        $http({
         url: '<?php echo base_url(); ?>client-getmembershipUsedData',
         method: "POST",
         headers: {
           'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
         }
          ...
          $scope.jobpostused = response.data.jobpost;
        ...
        $http({
         ...
         headers: {
          ...
          if(response.data.message == 'true')
          {
           ...
           $scope.jobpostmin = $scope.plan.jobpost;
           $scope.expense = $scope.plan.expense;
           $scope.invoice = $scope.plan.invoice;
           $scope.jobpost = $scope.plan.jobpost;
        ...
        $scope.clientplan = function ()
        {
          $http({
          url: '<?php echo base_url(); ?>admin/getclientcustom-package',
          method: "POST",
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
          }
          ...
          $scope.search = function()
          {
            ...
            $scope.changeinv = function ($event)
            {
               ...
               $scope.totalamount = $scope.expensetotal + $scope.invoicetotal +  $scope.jobposttotal;
                angular.element('.selectedinvoice').val($scope.invoice);
            ...
            $scope.changejobpost = function ($event)
            {
               ...
               $scope.jobposttotal = parseInt($scope.jobpostpaid) * parseInt($scope.jobpostprice);
               }
               else
               {
                 $scope.jobposttotal = 0;
