...
$http({
  ...
  if(response.data.messages.length > 0)
  {
    ...
    {
      var db = {};
      if(p1[a]['MSG_SENTBY'] == $scope.userId && p1[a]['MSG_SENTTO'] != $scope.userId && p1[a]['MSG_TYPE'] == 1 )
      {
        if(p1[a]['MSG_SENTTONAME'] && p1[a]['MSG_SENTBYNAME'])
        {
          db['cname'] = p1[a]['MSG_SENTTONAME'];
          db['cid'] = p1[a]['MSG_SENTTO'];
          db['clogo'] = p1[a]['MSG_SENTTOIMAGE'];
          db['fname'] = p1[a]['MSG_SENTBYNAME'];
          db['fid'] = p1[a]['MSG_SENTBY'];
          db['flogo'] = p1[a]['MSG_SENTBYIMAGE'];
          db['unread'] = 0;
        }
      ...
      else
      {
        if(p1[a]['MSG_SENTTONAME'] && p1[a]['MSG_SENTBYNAME'])
        {
          db['cname'] = p1[a]['MSG_SENTBYNAME'];
          db['cid'] = p1[a]['MSG_SENTBY'];
          db['clogo'] = p1[a]['MSG_SENTBYIMAGE'];
          db['fname'] = p1[a]['MSG_SENTTONAME'];
          db['fid'] = p1[a]['MSG_SENTTO'];
          db['flogo'] = p1[a]['MSG_SENTTOIMAGE'];
          db['unread'] = [];
        }
...
if($scope.name == '' || $scope.designation == '' || $scope.perhour == '' || $scope.image == '' ||  $scope.access == '' || $scope.status == '' || $scope.email == '')
{
  error = true;
}
// $scope.address == '' ||
...
angular.element(".preloader").show();
var obj = {  name  : $scope.name , desig  : $scope.designation,logo: $scope.image , maxPrice : $scope.perhour ,rep_ph_num : $scope.phone, address1  : $scope.address,services : $scope.skill, access :$scope.access ,email : $scope.email,status : $scope.status ,project :$scope.project,salary:$scope.salary,perday:$scope.perday,skype:$scope.skype,countryCode:$scope.countrycode,countryClass:cl,currencyId:$scope.currency,department:$scope.department,joiningdate:$scope.joiningdate };
...
$scope.messages[roomId] = [];
$http({
  url: 'http://localhost:5000/unread-msg?rec='+$scope.userId+'&&sender='+$scope.adminId+'&&status=0',
  method: "POST",
  headers: {
    'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
  }
...
app26.controller('myCtrt26', function($scope,$window,$http,$interval) {
  ...
  $scope.saveproject = function()
  {
    ...
    var obj = { projectName:$scope.projectname,hourlyPrice:$scope.hourlyprice,totalHour:$scope.totalhour,budget:$scope.budget,projectSource:$scope.projectsource,clientName:$scope.clientname,email:$scope.email,phone:$scope.phone,skype:$scope.skype,contactPerson:$scope.contactperson,projectManagerId:$scope.projectmanager,milestones:$scope.milestones,document:$scope.document,currency:$scope.currency,countrycode:$scope.countrycode,countryclass:$scope.countryclass   };
    angular.element('.preloader').show();
    $http({
...
<?php if(isset($projectId))
{
  ...
  app45.controller('myCtrt45', function($scope,$window,$http,$interval) {
    ...
    $scope.submittodo = function()
    {
      ...
      var error = false;
      $scope.date = angular.element('#date').val();
      if($scope.name == '' || $scope.priority == '' || $scope.department == '' || $scope.team == '' || $scope.description == ''  || $scope.date == '' || $scope.file == '')
      {
        error = true;
      }
    ...
    $scope.updatetodo = function()
    {
      ...
      if($scope.name1 == '' || $scope.priority1 == '' || $scope.department1 == '' || $scope.team1 == '' || $scope.description1 == '' || $scope.status1 == '' || $scope.date1 == '' || $scope.file == '' )
      {
        error = true;
      }
...
app53.controller('myCtrt53', function($scope,$window,$http,$interval) {
 ...
 $http({
  ...
  $scope.searchdate = function ()
  {
    ...
    var obj = {  startdate:$scope.startdate,enddate:$scope.enddate,sclient:$scope.sproject,sproject:$scope.sproject};
...
// jobopening
    ...
    $scope.edit = function(id)
    {
      ...
      $http({
        ...
        if(response.data.message == "true")
        {
           ...
           $scope.sendtoall1 = $scope.editdata.annSendAll;
           var users = [];
 ...
 $scope.get();
   ...
   $scope.save = function()
   {
     ...
     // || $scope.upload ==''
     if($scope.currency == '' || $scope.skype == '' || $scope.empName =='' || $scope.clientName == '' || $scope.phone =='' || $scope.email =='' || $scope.website =='' || $scope.jobtitle =='' || $scope.jobtitle =='' || $scope.jobdescription =='' || $scope.remark =='' || $scope.budget =='' || $scope.date =='' || $scope.status ==''  || $scope.leadcapture =='' || $scope.skill.length == 0)
     {
       error = true;
     }
   ...
   $scope.edit = function()
   {
     ...
     $http({
       ...
       if(response.data.message == "true")
       {
         ...
         $scope.leadcapture =$scope.editdata.leadCapture;
