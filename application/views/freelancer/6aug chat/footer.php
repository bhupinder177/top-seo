  ...
  $('body').on('click', '.buy_now', function(e){
    ...
    "handler": function (response){
          $.ajax({
            ...
            dataType: 'json',
            data: {
                razorpay_payment_id: response.razorpay_payment_id , totalAmount : totalAmount ,employee:employee,project:project,expense:expense,invoice:invoice,totalamount:totalAmount
            },
            dataType    : "json",
          beforeSend  : function ()
          {
...
{
var obj = {  page : $scope.page ,id : $scope.userid,perpage:$scope.perpage };
if(id )
obj['lastid'] = id;
...
<?php if(isset($projectId))
{
  ...
  app28.controller('myCtrt28', function($scope,$window,$http,$interval) {
    ...
    $scope.SaveMilestone = function()
    {
      ...
      if($scope.project.milestone)
      {
        for(var a in $scope.project.milestone)
        {
          ...
          {
            if($scope.project.milestone[a].task[b].checked == 1 && $scope.project.milestone[a].task[b].assignedTo && $scope.project.milestone[a].task[b].assignedTo != '' )
            {
    ...
    $scope.addsubtask = function ()
    {
      ...
      $scope.submitsubtask = function()
      {
        ...
        var obj = { name:$scope.name,status:'2',description:$scope.description,hours:$scope.hours,parents:$scope.ids };
        angular.element('.preloader').show();
        $http({
    ...
    $scope.changeStatus = function()
    {
      ...
      $http({
        ...
        if(response.data.success == "true")
        {
           if($scope.status == 1)
           {
           $.notify("Carry Forward Leave Activate Successfully", "success");
          }
          else if($scope.status == 0)
          {
            $.notify("Carry Forward Leave Deactivate Successfully", "success");
          }
  ...
  app45.controller('myCtrt45', function($scope,$window,$http,$interval) {
    ...
    $scope.updatetodo = function()
    {
      ...
      if($scope.name1 == '' || $scope.priority1 == '' || $scope.department1 == '' || $scope.team1 == '' || $scope.description1 == '' ||  $scope.date1 == '' || $scope.duedate1 == '' )
      {
        error = true;
      }
...
$scope.clickadvanceSearch =  function()
{
  ...
  $scope.submitinvoice = function()
  {
    $scope.submitl = true;
    var error = false;
    $scope.recurringType = angular.element('.recurring').val();
    $scope.duration = angular.element('.duration').val();
...
if($scope.atotal > 10)
{
 ...
 $scope.changereceived = function ($event)
 {
   ...
   }).then(function(response)
   {
      if(response.data.success == "true")
       {
        $.notify("Invoice Changed Payment Mode Successfully", "success");
        $scope.getinvoice();
...
$scope.dsr.push({
    ...
    $scope.keyupcounthour = function($event)
    {
      var typingTimer = (new Date()).getTime()
    ...
    $scope.counthours = function()
    {
      if($scope.dsr)
      {
       ...
       h = parseInt($scope.totalhours / 60);
      if(h < 10)
      {
        h = '0'+h;
      }
      else if(h == 0)
      {
       ...
       m = parseInt($scope.totalhours % 60);
      if(m < 10)
      {
        m ='0'+m;
      }
...
{
  var obj = { page :$scope.page,startdate:$scope.startdate,enddate:$scope.enddate };
  $http({
...
$scope.savedsr = function()
{
  ...
  if($scope.dsr)
  {
    for(var i in $scope.dsr)
    {
      if($scope.dsr[i].projectId == '' || $scope.dsr[i].task == '' || $scope.dsr[i].taskDetail == '' || $scope.dsr[i].status == '' || $scope.dsr[i].time == '' )
      {
        error = true;
      }
...
$scope.dsrhistory = function (userId,todoId,taskId,date)
{

   var obj = { userId:userId,date:date,todoListId:todoId,taskId:taskId};
   angular.element('.preloader').show();
   $http({
     url: '<?php echo base_url(); ?>freelancer/getdsrhistoryBilling',
     method: "POST",
     data: obj,
     headers: {
       'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
...
$scope.pagination = function ($event)
{
  ...
  $scope.changeemp = function ($event)
  {
     ...
     $scope.totalamount = $scope.employeetotal + $scope.projecttotal + $scope.expensetotal + $scope.invoicetotal;
     angular.element('.selectedemployee').val($scope.employee);
...
<?php } ?>
   ...
   <?php if(!empty($gigId))
      ...
      app83.directive('mydatepicker', function () {
        return {
          restrict: 'A',
          require: 'ngModel',
          link: function (scope, element, attrs, ngModelCtrl) {
...
<?php if(!empty($gigassignId))
 ...
 $scope.taskChecked = function(mkey,key,$event)
      ...
      $scope.SaveMilestone = function()
    {
      ...
      if($scope.gig.milestone)
      {
        for(var a in $scope.gig.milestone)
        {
          for(var b in $scope.gig.milestone[a].task)
          {
            if($scope.gig.milestone[a].task[b].checked == 1 && $scope.gig.milestone[a].task[b].assignedTo && $scope.project.milestone[a].task[b].assignedTo != '' )
            {
              var id = $scope.gig.milestone[a].task[b].id;
              $scope.gig.milestone[a].task[b].startDate = angular.element('.startdate'+id).val();
              $scope.gig.milestone[a].task[b].dueDate = angular.element('.enddate'+id).val();
              $scope.gig.milestone[a].task[b].saved = 1;
              error = true;
            }
          }
        }
    ...
    $scope.AssignMilestone = function()
    {
      ...
      if($scope.gig.milestone)
      {
        for(var a in $scope.gig.milestone)
        {
          ...
          {
            // if($scope.project.milestone[a].task[b].checked == 1 && $scope.project.milestone[a].task[b].assignedTo && $scope.project.milestone[a].task[b].assignedTo != '' )
            if($scope.gig.milestone[a].task[b].checked == 1 )
            {
              var id = $scope.gig.milestone[a].task[b].id;
              $scope.gig.milestone[a].task[b].startDate = angular.element('.startdate'+id).val();
              $scope.gig.milestone[a].task[b].dueDate = angular.element('.enddate'+id).val();
              $scope.gig.milestone[a].task[b].saved = 0;
              error = true;
            }
          }
        }
