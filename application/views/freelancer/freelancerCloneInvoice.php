...
<section class="content portfolio-cont bg-white">
  <div class="no-margin user-dashboard-container">
        <div ng-cloak  ng-app="myApp81" ng-controller="myCtrt81">
          <div id="content">
                 <!-- main invoice page -->
                 <div id="page-wrap">

    <p>Clone Invoice {{ invoiceno }}</p>
  ...
  <div class="row">
    <div class="col-md-2">
     Recurring <input ng-model="showrecurring" ng-checked="showrecurring == 1" value="1" type="checkbox" >
    </div>
    ...
    <div ng-if="recurringevery == 3" class="col-md-2">
      <div class="form-group">
    ...
    <div ng-if="showrecurring == 1" class="col-md-2">
     Ending <input value="1" ng-checked="end == 1" ng-click="endclick($event)" ng-model="end" type="checkbox" >
    </div>
    <div ng-if="end == 1" class="col-md-1">
      <input type="radio" name="never"  ng-model="endNever" ng-click="clickendNever(1)" ng-value="endNever"  value="1" >Never
    </div>
    <div ng-if="end == 1" class="col-md-1">
      <input type="radio" name="never" ng-model="endAfter" ng-click="clickendNever(2)" ng-checked="endAfter == 1" ng-value="endAfter" value="1" >After
    </div>
    <div ng-if="end == 1" class="col-md-1">
      <div class="form-group">
      <input type="text" ng-model="Afterduration" ng-value="Afterduration" class="afterduration form-control" ng-model="never" value="" >Occurrence(s)
      </div>
    </div>
  </div>
  <!-- recurring -->
...
<div class="row">
    <div class="col-md-4">
        <div class="form-group">
    <select ng-disabled="checked" id="address" onchange="angular.element(this).scope().getcontract(this)" id="client" class="form-control" name="recipient" ng-model="recipient" >
    ...
    <div class="col-md-4">
         <div class="form-group">
        <input class="form-control discount" ng-keyup="counttotalhour()" ng-readonly="checked" numbers-only="numbers-only" ng-if="discounttype !=''" ng-keyup="counttotalhour()" class="discount form-control" id="address"   ng-model="discount" ng-value="discount" placeholder="Please enter discount">
        </div>
        </div>
    <div class="col-md-4">
     <div class="form-group">
...
<span id="error-msg" class="hide"></span>
  ...
  <option ng-repeat="(key,x) in contactmilestone" data-id="contract" value="{{ x.projectId	 }}">{{ x.projectName }}</option>
  </select>
...
<option ng-repeat="(key,x) in ownmilestone" data-id="own" ng-selected="x.id == selectedmilestone" value="{{ x.id	 }}">{{ x.name }}</option>
  ...
  <table id="items" class="table">
    ...
    <tr class="item-row" ng-repeat="(key2,x2) in task">
        ...
        <td>
          <input class="form-control" ng-readonly="checked" numbers-only="numbers-only" ng-keyup="counttotalhour()" ng-model="x2.hourly" ng-value="x2.hourly"  class="cost">
          <p ng-cloak ng-show="submitl && x2.hourly == ''" class="text-danger">Please enter hourly price.</p>

        </td>
        <td>
          <input class="form-control" ng-readonly="checked" numbers-only="numbers-only" ng-keyup="counttotalhour()" ng-model="x2.hours" ng-value="x2.hours"  class="qty">
    ...
    <tr>
        ...
        <td class="total-value form-group"><div id="paid" class="form-control">{{ discountAmount }}</div></td>
    </tr>
    <tr>
