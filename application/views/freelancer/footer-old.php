...
app9.controller('myCtrt9', function($scope,$window,$http) {
    ...
    $scope.profile.desig = angular.element("#desig").val();
	  ...
	  $scope.profile.pinterestLink =angular.element("#pinterest").val();
		...
		if($scope.profile.address1 == '' || $scope.profile.address2 == '' || $scope.profile.about == '' || $scope.profile.minPrice == '' || $scope.profile.maxPrice == '' || $scope.profile.c_name =='' || $scope.profile.logo == '' )
		{
			error = true;
		}
		//console.log(error);
...
// $scope.phone == '' || $scope.address == '' ||
		if($scope.name == '' || $scope.designation == '' || $scope.hourly == '' || $scope.image == '' ||  $scope.access == '' || $scope.status == '' || $scope.email == '')
		{
			error = true;
		}
		if(error == true)
	...
	$scope.updateteam = function (id)
	{
		...
		var error = false;
// || $scope.phone1 == '' || $scope.address1 == ''
		if($scope.name1 == '' || $scope.designation1 == '' || $scope.hourly1 == ''   || $scope.access1 == '' || $scope.skill1 == '' || $scoep.status1 == '' )
		{
			error = true;
		}
		if(error == true)
		...
		angular.element(".preloader").show();
		var obj = {  name  : $scope.name1 , desig  : $scope.designation1,logo : $scope.image , maxPrice : $scope.hourly1 ,services : $scope.skill1, access :$scope.access1,id:$scope.editTeam.id,status:$scope.status1 };
		$http({
...
app24.controller('myCtrt24', function($scope,$window,$http,$interval) {
  ...
  $scope.referencelabels = [];
	...
	$scope.getexpensekeyup = function()
	{
	  $scope.services =[];
	  var ex = angular.element('#expense').val();

	  if(ex != '')
	  {
	    var m = JSON.stringify({ name : ex });
	   $http({
	   url: '<?php echo base_url(); ?>freelancer/expenseSearch',
	   method: "POST",
	   data:m,
	   headers: {
	    'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
	  ...
	  $scope.referencelabels = response.data.result;
